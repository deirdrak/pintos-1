...
/****** Project SisO1 2017 only ******/
static struct semaphore * semaphores;
struct lock * mylock;

int sys_semarray(int semCant);
int sys_seminit(int index, int value);
int sys_semwait(int index);
int sys_sempost(int index);

int sys_lockarray(int lCant);
int sys_lockinit(int index);
int sys_lockacquire(int index);
int sys_lockrelease(int index);
/**************************************/
void
syscall_init (void)
{
...
syscall_handler (struct intr_frame *f)
{
  ...
  case SYS_INUMBER: // 19
	...
	case SYS_SEMINIT: //20
	{
		int index;
    	int value;
    	memread_user(f->esp + 4, &index, sizeof(index));
    	memread_user(f->esp + 8, &value, sizeof(value));
    	f->eax = sys_seminit(index,value);
		break;
	}
	...
	case SYS_SEMCANT://21
	{
    	...
    	memread_user(f->esp + 4, &cant, sizeof(cant));
    	f->eax = sys_semarray(cant);
		break;
	}
	...
	case SYS_SEMPOST://23
	{
    	...
    	memread_user(f->esp + 4, &index, sizeof(index));
    	f->eax = sys_sempost(index);
		break;
	}

	case SYS_LOCKINIT://24
	{
		lock_init (&mylock);
		break;
	}

	case SYS_LOCKACQUIRE://25
	{
		lock_acquire(&mylock);
		break;
	}

	case SYS_LOCKRELEASE://26
	{
		lock_release(&mylock);
		break;
	}

	case SYS_LOCKCANT://27
	{
		int cant;

    	memread_user(f->esp + 4, &cant, sizeof(cant));
    	f->eax = sys_lockarray(cant);
		break;
	}
#endif
...
int sys_semarray(int semCant)
{
  int value = semCant * sizeof(struct semaphore);
...
int sys_seminit(int index, int value)
{
  sema_init(&semaphores[index],value);
  return 0;
}

int sys_semwait(int index)
{
  sema_down(&semaphores[index]);
...
int sys_lockarray(int lCant)
{
	int value = lCant * sizeof(struct lock);
    mylock = malloc(value);
...
int sys_lockinit(int index)
{
	lock_init(&mylock[index]);
	return 0;
}

int sys_lockacquire(int index)
{
	lock_acquire(&mylock[index]);
	return 0;
}

int sys_lockrelease(int index)
{
	lock_release(&mylock[index]);
	return 0;
}
